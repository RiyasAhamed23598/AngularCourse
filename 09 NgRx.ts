...
// Structure of the 1st parameter (mandatory) in createAction():

// It's a string that represents the type of the action. The naming convention:
"[Source or Category] Event or Action Description"
// [Source or Category] (within square brackets) usually indicates the feature or module where the action is used. 
// Event or Action Description (without square brackets) describes the specific event or action that is happening.
// This naming convention helps organize actions, making it easier to identify where an action is being dispatched from and what it does.

// Each part (inside the brackets or after them) can consist of many words to describe the feature and the action comprehensively. Examples:
...
// It also reduces the likelihood of action type collisions in large applications.

// The 2nd parameter (optional) to inforce strong typing - props<T>()
...
// More examples:
export const updateUserProfile = createAction('[User Profile Page] Update User Profile', props<{ userId: string; changes: Partial<UserProfile> }>());
export const addUser = createAction('[User API] Add User', props<{ id: string; name: string; email: string }>());

// Data Type Returned by createAction():

// The createAction() function returns a function of type ActionCreator which represents the shape of an action creator function.
// When that ActionCreator function is called, it returns an Action object with a specific type and optionally a payload.

// TypedAction is a utility type used in NgRx to represent actions with a specific type:
export interface TypedAction<T extends string> {
  readonly type: T;
}

// ActionCreator is defined as a type alias and represents both a function signature and an object with specific properties (type):
type Creator = (...args: any[]) => object;
type ActionCreator<T extends string, C extends Creator> = C & TypedAction<T>;

// The generic parameters represent the parameters passed to createAction().

// The "C & TypedAction<T>" fragment defines a function return type. It indicates that the ActionCreator can be called as a function.
// The function returns an object that combines the payload shape (C) with the "type" property (forced by TypedAction<T> interface).

// The next example shows how the shape of the returned action object is built dynamically:
export const addUser = createAction('[User API] Add User', props<{ id: string; name: string; email: string }>());
// The object returned by createAction is an ActionCreator function.
// This function, when called with the appropriate payload, returns an action object with this shape:
{
  type: string;
  id: string;
  name: string;
...
// Reducers are central to NgRx's state management model, ensuring that state transitions are predictable and traceable.
...
// It should be regarded as the one and only single source of data that is delivered to the application.
